...
#alias allprojects='ls -d $ADDONS_PATH_PREFIX/odoo-*'
...
function _cdprojectdir() {
    [ -z $1 ] || ODOOPROJECT=$1
    cd $ADDONS_PATH_PREFIX/$ODOOPROJECT
}
alias cdp='_cdprojectdir'

alias cdo="cd $ADDONS_PATH_PREFIX/odoo-addons"

function _cdcore(){
  source /etc/odoo/odoo.tools
  cd $ODOOCORE
}
alias cdcore=_cdcore
...
# function _odoofind() {
#     find -L $ADDONS_PATH_PREFIX/odoo-addons -type f -exec grep -Hn $1 {} \;
# }
# alias odoofind='_odoofind'

# function _odoolistprojects() {
#     local PROJECTS=""
#     for PROJECT in $(ls $ADDONS_PATH_PREFIX/odoo-* | grep -v odoo-addons); do
#         PROJECTS+=$PROJECT,
#     done
#     echo "${PROJECTS::-1}"
# }
